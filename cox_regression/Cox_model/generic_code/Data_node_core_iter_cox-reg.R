...
data_iter_cox_reg <- function(man_wd, nodeid, iterationseq, robflag, expath) {
  ...
  Rik <- read.csv(paste0(examplefilepath, "Rik", k, ".csv"), header = FALSE, blank.lines.skip = FALSE)
  Rik <- Rik[-1, ]
  Rik_comp <- read.csv(paste0(examplefilepath, "Rik_comp", k, ".csv"), header = FALSE, blank.lines.skip = FALSE)
  Rik_comp <- Rik_comp[-1, ]
