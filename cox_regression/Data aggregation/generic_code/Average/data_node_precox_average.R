...
data_node_precox_average <- function(man_wd=-1,man_nodeid=-1,bucketsize=-1) {
...
# Function to create groups with at least bucketsize rows where status == 1
create_groups <- function(data, fctbucketsize) {
  data <- data %>%
  ...
  if (sum(data$group == last_group & data$status == 1) < fctbucketsize) {
    data <- data %>%
...
node_data_grouped <- create_groups(node_data, bucketsize)
