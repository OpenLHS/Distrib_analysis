...
linear_regression_example_coordnode_handler <- function(man_wd=-1, expath=""){
  ...
  K=length(list.files(path = examplefilepath, pattern="Node[[:digit:]]+_output.csv"))
  ...
  for (k in 1:K) {
    output_k <- read.csv(paste0(examplefilepath, "Node", k, "_output.csv"))
  ...
  write.csv(output, file=paste0(examplefilepath, "CoordNode_results_distributed_lin_reg.csv"))
