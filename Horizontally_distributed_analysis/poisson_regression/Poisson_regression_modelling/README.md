# Horizontally distributed Poisson regression model 

This implementation of the poisson regression model mimics the following `R` calls: 
- `glm(formula, data, family="poisson", weights)`, whenever one checks `Coord_node_iter_t_results.csv` and once convergence is attained. 
...
Assuming a data structure similar to the data nodes `.csv` files in the example folder, this code can be used to execute a data node operation or a coordination node operation for a horizontally distributed poisson regression.
...
***If you are not able to automatically set your working directory (for example, if you do not have access to `this.path`), manually set the variable `manualwd = 1` in `Data_node_call_poi-reg_k.R` and  `Coord_node_call_iter_poi-reg.R`.***
...
1. Run the local `R` file (`Data_node_call_poi-reg_k.R`) for each data node to compute local settings and local beta estimates.  
The files `Data_node_k_iter_0.csv` and `Predictor_names` will be generated. All files must be sent to the coordination node.

2. Run the coordination `R` file (`Coord_node_call_iter_poi-reg.R`) to initialise the values of beta.  
...
3. Run the local `R` file (`Data_node_call_poi-reg_k.R`) for each data node to compute local aggregates used for derivatives.  
...
4. Run the coordination `R` file (`Coord_node_call_iter_poi-reg.R`)to compute first and second derivative and to update beta estimate.  
...
5. Run the local `R` file (`Data_node_call_poi-reg_k.R`) for each data node to compute local aggregates used for derivatives.  
The file `Data_node_k_iter_(t).csv` will be generated. It must be sent to the coordination node.

6. Run the coordination `R` file (`Coord_node_call_iter_poi-reg.R`)to compute first and second derivative and to update beta estimate.  
