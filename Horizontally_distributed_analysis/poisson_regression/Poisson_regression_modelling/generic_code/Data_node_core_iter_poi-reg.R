...
data_iter_poi_reg <- function(man_wd, nodeid, iterationseq, expath="") {
...
# Poisson-regression-specific functions
poireg_D <- function(beta, X, y, W) {
  t(X) %*% W %*% (y - exp(X %*% beta)) # / n
}

poireg_V <- function(beta, X, W) {
...
D_k_t <- poireg_D(beta_t, X_k, y_k, W_k)
V_k_t <- poireg_V(beta_t, X_k, W_k)
...
write.csv(output,
          file=paste0(examplefilepath, "Data_node_", k, "_iter_", t, "_output.csv"), row.names=FALSE)
