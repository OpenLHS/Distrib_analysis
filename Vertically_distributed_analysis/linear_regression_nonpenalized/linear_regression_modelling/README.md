# Vertically distributed penalized linear regression (VALINRIS)

This implementation of the Vertical Linear Regression leads to valid estimates and standard errors for the linear regression model (with no penalization).
...
- The implementation was tested to operate using `R` version 4.5.2.
...
2. `simulated_data_same_folder`  
	This folder contains the simulated dataset obtained from running the `DataNodeDistrib_generation.R` file. This folder makes it easy to look at the output since everything is happening in the same folder. There is no need to copy files across folders.

### Generic code

The files in this folder can be used to execute a vertically distributed linear regression analysis. The generic code assumes a data structure with one response-node holding the response-vector (and potentially covariates) and covariate-nodes holding only covariates. This can can be used to execute a covariate-node operation or a response-node operation for a vertically distributed linear regression.
...
***If you are not able to automatically set your working directory (for example, if you do not have access to `this.path`), manually set the variable `manualwd = 1` in `Response_node_call_iter_lin-regV.R` and  `Data_node_call_lin-regV.R`.***
...
1. Run the covariate-node `R` file (`Data_node_call_lin-regV.R`) for each covariate-node to compute local gram matrices.  
...
2. Run the response-node `R` file (`Response_node_call_iter_lin-regV.R`) to compute intermediary quantities for the covariate-nodes and, if any, to obtain parameter estimates and standard errors asssociated with covariates at the response-node.  
...
3. Run the covariate-node `R` file (`Data_node_call_lin-regV.R`) for each covariate-node to compute parameter estimates and standard errors associated with the covariates it holds.  
