...
vert_linear_regression_example_datanode_handler <- function(man_wd=-1, nodeid=-1, expath=""){
  ...
  if (nbprimerfilesA == 1 & nbprimerfilesB == 1) {
    source("../../generic_code/Data_node_call_iter_lin-regV.R")
    data_call_iter_lin_reg(manualwd,manualk,examplefilepath)
  } else if(nbprimerfilesA == 0 & nbprimerfilesB == 0) {
    source("../../generic_code/Data_node_call_init_lin_regV.R")
    data_call_init_lin_reg(manualwd,manualk,examplefilepath)} else {
