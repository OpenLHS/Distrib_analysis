...
nbprimerfilesA <- length(list.files(path=path, pattern=paste0("Coord_node_primerA_for_data_node_", manualk, ".csv")))
nbprimerfilesB <- length(list.files(path=path, pattern=paste0("Coord_node_primerB_for_data_node_", manualk, ".rds")))
if (nbprimerfilesA == 1 & nbprimerfilesB == 1) {
  source("Data_node_call_iter_lin-regV.R")
  data_call_iter_lin_reg(manualwd,manualk,path)
} else if(nbprimerfilesA == 0 & nbprimerfilesB == 0) {
  source("Data_node_call_init_lin_regV.R")
  data_call_init_lin_reg(manualwd,manualk,path)} else {
