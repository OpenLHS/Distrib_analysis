...
- `glmnet_model <- glmnet(x, y, family=binomial, lambda=cvfit$lambda.min, alpha=0)`
...
The files in this folder can be used to execute a vertically distributed ridge logistic regression analysis.
...
to obtain the unscaled intercept estimate at the response-node and, if any, to obtain parameter estimates and standard errors asssociated with predictors at the response-node.  
