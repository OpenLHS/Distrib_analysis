...
privacy_check_ck2 <- function(V,alpha_tilde,y,n,i0){
  # Compute which entries of y can be flipped
  ...
  constraintF <- F_constraint(
    F   = function(x) as.numeric(sum(x0) + sum(V %*% x)),
    J   = function(x) grad_vec,   # constant gradient
    dir = "==",
    rhs = 0
  )
  
  constraintL <- L_constraint(L = as.matrix(A_total), dir = dir_total, rhs = rhs_total)
  
  L <- OP(objective = rep(0,ncol(V)), constraints = c(constraintF,constraintL))
  
  gc()
  
  res2 <- ROI_solve(L, solver = "nloptr.slsqp",start = rep(0,ncol(V)))
  
  x_opt <- solution(res2)
  x_opt
  sol_flip <- x0 + V%*% solution(res2)
  #sol_flip <- x0 + arma_mm(V,solution(res2))
  sum(sol_flip)
  
  
  if(res2$status$code == 0){
    sol <- x0 + V%*% solution(res2)
    #sol <- x0 + arma_mm(V,solution(res2))
    return(sol)
  }else{return(NULL)}
}
