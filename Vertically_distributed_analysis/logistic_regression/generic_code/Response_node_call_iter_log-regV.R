...
nb_node1_files <- length(list.files(pattern="Data_node_1.csv"))
nb_node_output_files <- length(list.files(pattern="Data_node_[[:digit:]]+_init_output.rds"))

if (nb_node1_files==1 & nb_node_output_files>0) {
  source("Response_node_init_iter_log-regV.R")
  coord_log_reg(man_wd = manualwd, man_lambda = lambda)
} else {
