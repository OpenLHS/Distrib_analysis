...
This implementation of the Vertical Ridge-Penalized Binary Regression leads to valid estimates for the ridge-penalized probit regression model.
Provided a binomial family (family_glm) taking the value "probit" and a lambda sequence (lambda_seq), the results can be interpreted as they would with the following `R` calls in a pooled setting: 
...
- generic_code_probit
The generic_code_probit folder contains `R` code files pertaining to the vertical approach for the ridge-penalized probit regression model. Please read the code and its comments in the `R` file as files may require edition before being used.
...
Response-node (packages required when running the privacy assessment):
- [Rcpp](https://cran.r-project.org/web/packages/Rcpp/index.html)
- [RcppArmadillo](https://cran.r-project.org/web/packages/RcppArmadillo/index.html)
...
***If you are not able to automatically set your working directory (for example, if you do not have access to `this.path`), manually set the variable `manualwd = 1` in `Response_node_call_iter_bin-regV_probit.R` and  `Data_node_call_bin-regV_probit.R`.***

In the following procedure, `k` represents the number of the predictor-node. In the Data files, Node `1` must be associated with the response-node. The `R` files mentioned in the procedure are found in the folder generic_code_probit. 

Initialization:

1. Run the predictor-node `R` file (`Data_node_call_bin-regV_probit.R`) for each predictor-node to compute local gram matrices.  
...
2. Run the response-node `R` file (`Response_node_call_iter_bin-regV_probit.R`) to compute intermediary quantities for the predictor-nodes,
...
When running this code, indicate if you want to run the data-driven privacy validation at the response-node (default is YES). When the privacy validation is run, the file `privacy_output.csv` will be generated from the estimation procedure and used in the validation. 
The latter file SHOULD NOT be shared outside the response-node.


For the single iteration, predictor-node side:

3. Run the predictor-node `R` file (`Data_node_call_bin-regV_probit.R`) for each predictor-node to compute parameter estimates and standard errors associated with the predictors it holds.  
...
This implementation leads to valid estimates for the ridge-penalized probit regression model.
The results can be interpreted as they would with the following `R` calls in a pooled setting, with family_glm taking the value "probit": 
