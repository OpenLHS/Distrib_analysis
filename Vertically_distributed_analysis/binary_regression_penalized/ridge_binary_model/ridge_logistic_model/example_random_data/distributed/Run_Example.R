...
if (nb_node1_files==1 & nb_node_output_files>0) {
  ...
  response_bin_reg(man_lambda = lambda, expath = examplefilepath, model_choice = "logit")
  
  if(privacy==1){
    setwd(this.dir())
    setwd("../../generic_code_logistic")
    if(gurobi==0){
      source("Response_node_optional_confidentiality_logistic.R")
    }else if(gurobi==1){
      source("Response_node_optional_confidentiality_logistic_gurobi.R")
    }
