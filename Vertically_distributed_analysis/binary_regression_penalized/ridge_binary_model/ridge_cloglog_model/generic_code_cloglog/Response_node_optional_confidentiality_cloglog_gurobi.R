...
')
  ...
  privacy_check_ck2 <- function(V,alpha_new,n,i0){
    ...
    # Box constraints: x0 + V b in (-infinity,1)^n
    dir_box <- rep("<=", n)
    rhs_box <- c(1 - x0-1e-4)
    ...
    model$A          <- rbind(V, matrix(V[i, ], nrow = 1),grad_vec)
    model$obj        <- rep(0,ncol(V))
    model$modelsense <- 'min'
    model$rhs        <- c(rhs_box, rhs_flip, rhs_equality)
    model$sense      <- c(dir_box, dir_flip, "=")
    model$vtype      <- 'C'
  ...
  #Ratios for the cloglog model
  alpha_new[which(y==1)] <- (exp(eta[which(y==1)]-exp(eta[which(y==1)])))/(1-exp(-exp(eta[which(y==1)])))
  alpha_new[which(y==0)] <- -(exp(eta[which(y==0)])) 
  gc()
  y <- sign(alpha_new)
