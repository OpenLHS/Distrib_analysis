...
1. `random_data_same_folder` contains a single response node. It can be used to see what information is splitted when running the method.
...
1.	Open the file `Run_Example.R`. It should then appear in `R`.
2.	Select all the code and click `run`.
3.	All iterations results will be saved in `.csv` files.
